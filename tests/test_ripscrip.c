...
/* ═══════════════════════════════════════════════════════════════════
 * BRANCH COVERAGE — rip_tw_putchar control characters, scrolling,
 * wrapping. Activates the extended text window then feeds raw bytes
 * that bypass the !| escape and route through the tw_active branch.
 * ═══════════════════════════════════════════════════════════════════ */

static void tw_activate_window(rip_state_t *s, comp_context_t *ctx) {
    /* |b activates extended text window.  Format:
     *   x0:2 y0:2 x1:2 y1:2 fore:2 back:2 font:1 size:4 flags:3
     * Use x0=20 y0=20 x1=200 y1=100 fore=15 back=0 font=0 size=10 flags=0.
     * "0K"=20, "5K"=200, "2S"=100, "000A"=10.
     * Trailing \n returns the FSM to IDLE so subsequent raw bytes go
     * through the tw_active branch rather than being parsed as commands. */
...
static void test_tw_carriage_return_resets_cursor(void) {
    ...
    /* Push 'A' then '\r' then check cursor returns to tw_x0. */
    rip_process(&s, &ctx, 'A');
...
static void test_tw_tab_advances_to_next_stop(void) {
    ...
    rip_process(&s, &ctx, '\t');
    /* Tab advances to next 8-col boundary; from x=x0 (col 0) → 8 chars = 64px */
...
static void test_tw_control_char_ignored(void) {
    ...
    if (!s.tw_active) { FAIL("setup"); return; }
    int16_t before = s.tw_cur_x;
    rip_process(&s, &ctx, 0x05);  /* ENQ — should be silently dropped */
...
/* ═══════════════════════════════════════════════════════════════════
 * BRANCH COVERAGE — rip_expand_variables: built-in $XXXX$ variables
 * that had no dedicated test. Verified through 1Q-equivalent paths
 * or side-effect TX captures.
 * ═══════════════════════════════════════════════════════════════════ */
...
/* Drawing tests below use X<xx>00 so the Y argument scales to 0 and
 * we don't need to compute scale_y(y) in the verification. */
...
/* ═══════════════════════════════════════════════════════════════════
 * BRANCH COVERAGE — rip_draw_icon_pixels modes 0/2/3
 * (mode 1 / tile is already tested elsewhere).
 * ═══════════════════════════════════════════════════════════════════ */
...
static void test_icon_style_stretch_mode(void) {
    ...
    /* |1I loads icon at (0,0) — style box overrides destination */
    feed_script(&s, &ctx, "!|1I000000000MINI|");
    /* Stretched mini icon should fill (5..25, 5..25). Sample center. */
...
static void test_icon_style_center_mode(void) {
    ...
    /* style box: x0=10 y0=10 x1=50 y1=50, mode=02 (center), align=00.
     * "1E" = 1*36+14 = 50.  After scale: y0=11, y1=scale_y1(50)=58.
     * Centering a 2×2 icon in a 41×48 box → top-left at (29, 34). */
...
static void test_icon_style_proportional_mode(void) {
    ...
    /* style box (10,10)-(50,30), mode=03, align=00 */
    feed_script(&s, &ctx, "!|&0A0A1E1E0300000000|");
    feed_script(&s, &ctx, "!|1I000000000WIDE|");
    /* Proportional fit: 41 wide × 20 tall.  Aspect is 2:1, so width
     * dominates → fitted dims ~41×20 (already capped by h). Some pixel
     * inside the box should be filled. */
    int found = 0;
    for (int y = 10; y < 30 && !found; y++)
...
int main(void) {
    ...
    /* Branch-coverage tests for the lowest-coverage internal functions */
