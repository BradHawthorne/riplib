...
| U-024 | Ground truth for `1M` mouse reserved field: did the original DLL/RIPterm consume 11 chars (spec §3.2) or 17 (code, with res:4)? Decides whether C-012 fixes the code or amends the spec | 2026-05-30 | open | — Needs DLL/RIPterm disassembly or a real `1M` wire capture. |
| U-025 | Ground truth for `1D` DEFINE wire grammar: does the real DLL carry the `flags:3 res:2` prelude the code expects, or the bare `name=value` of spec §3.18? | 2026-05-30 | open | — Same source as U-024. |
